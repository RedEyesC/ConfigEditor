...
生成参数有5个，第一个参数指定读取的配置定义文件，第二个参数指定需要生成的配置表所在根目录， 第三个参数指定需要生成的配置表名字，支持同时生成多个配置表，用 | 分割配置表名，例如config1|config2，第四个参数指定生成出来的配置的根目录，第五个参数指定生成出来的配置的格式，支持json和bin两种 ，支持同时输出多种， | 分割，例如json|bin
